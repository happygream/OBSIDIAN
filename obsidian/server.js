...
const TOOL_META = {
  ...
  testssl:     { binary:'testssl.sh',  name:'testssl.sh',   desc:'TLS/SSL configuration tester',                          install:'apt:testssl.sh' },
  ...
  // New tools v0.3.0
  rustscan:    { binary:'rustscan',    name:'rustscan',     desc:'Ultra-fast port scanner — feeds results to nmap',       install:'cargo-direct:rustscan' },
  ...
  feroxbuster: { binary:'feroxbuster', name:'feroxbuster',  desc:'Recursive web content discovery',                      install:'cargo-direct:feroxbuster' },
  arjun:       { binary:'arjun',       name:'arjun',        desc:'HTTP parameter discovery tool',                         install:'pip3 install arjun', pipPkg:'arjun' },
  enum4linux:  { binary:'enum4linux',  name:'enum4linux',   desc:'SMB/NetBIOS enumeration for Windows targets',          install:'https://github.com/CiscoCXSecurity/enum4linux' },
  ...
  crackmapexec:{ binary:'crackmapexec',name:'crackmapexec', desc:'SMB/AD testing — pass-the-hash, shares enumeration',   install:'pipx:crackmapexec' },
...
function checkTools(ws, manual) {
  ...
  if (IS_WIN && WSL_AVAILABLE) {
    ...
    const pyLines = [
      ...
      'os.environ["PATH"]=h+"/.local/bin:"+h+"/go/bin:"+h+"/.cargo/bin:/usr/local/bin:/usr/bin:/bin:/usr/games:/var/lib/gems/3.3.0/bin:/var/lib/gems/3.3.0/wrappers:"+os.environ.get("PATH","")',
      'pip_ok=lambda p:sp.run(["pip3","show",p],capture_output=True,timeout=5).returncode==0',
      'def found(b):',
      '    if shutil.which(b): return True',
      '    for d in ["/usr/bin","/usr/local/bin",h+"/.local/bin",h+"/go/bin",h+"/.cargo/bin","/var/lib/gems/3.3.0/bin","/var/lib/gems/3.3.0/wrappers","/usr/share/testssl.sh"]:',
      '        if os.path.isfile(os.path.join(d,b)): return True',
      '    return False',
    ];
    entries.forEach(([k, m]) => {
      const bin = m.binary.replace(/[^a-zA-Z0-9._-]/g, '');
      const pip = m.pipPkg ? `pip_ok("${m.pipPkg}")` : 'False';
      pyLines.push(`print("${k}="+("1" if (found("${bin}") or ${pip}) else "0"))`);
    });
    const pyB64 = Buffer.from(pyLines.join('\n')).toString('base64');
    let out = '';
    const proc = spawn('wsl.exe', ['-d', WSL_DISTRO, 'python3', '-c',
      `import base64; exec(base64.b64decode("${pyB64}").decode())`
    ], { stdio: ['ignore','pipe','pipe'], shell: false });
    proc.stdout.setEncoding('utf8');
    ...
    proc.on('close', () => {
      const installed = {};
      out.split('\n').forEach(line => {
        const m = line.trim().match(/^([a-z0-9_]+)=([01])$/);  // key is always the TOOL_META key (no dots)
      ...
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: 'toolStatus', results, wsl: { available: WSL_AVAILABLE, distro: WSL_DISTRO }, isWin: IS_WIN, fromRefresh: !!manual }));
        const cnt = Object.values(results).filter(r => r.installed).length;
        ws.send(JSON.stringify({ type: 'log', module: 'system', logType: 'ok',
          text: '[+] Tool status refreshed — ' + cnt + '/' + entries.length + ' tools installed' }));
      }
    });
    proc.on('error', () => {
      const results = {};
...
function installTool(key, ws) {
  ...
  if (inst.startsWith('http')) {
    ...
    shellCmd = 'pip3 install pipx --break-system-packages 2>/dev/null; pipx install ' + pipxPkg + ' 2>/dev/null && ln -sf $HOME/.local/bin/' + meta.binary + ' /usr/local/bin/' + meta.binary + ' 2>/dev/null || true';
    ...
    // cargo and PATH are set in the bash script shebang context
    shellCmd = 'PATH=$HOME/.cargo/bin:$PATH cargo install ' + cargoPkg
  ...
  if (IS_WIN && WSL_AVAILABLE) {
    ...
    const needsRoot = isApt || shellCmd.startsWith('gem install') || shellCmd.startsWith('apt-get install -y python3-venv');
    const user = needsRoot ? ['-u', 'root'] : [];
    const runCmd = 'T=/tmp/obs_' + Date.now() + '.sh; echo ' + JSON.stringify(scriptB64) + ' | base64 -d > $T && chmod 777 $T && bash $T; rm -f $T 2>/dev/null';
...
function startServer(port) {
  return new Promise((resolve) => {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.send(JSON.stringify({ type: 'connected', version: '0.4.1' }));
