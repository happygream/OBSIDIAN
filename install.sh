...
# ---- New tools v0.3.0 ----
log "Installing new tools..."

# rustscan
if command -v rustscan &>/dev/null; then
  ok "rustscan already installed"
else
  warn "rustscan not found — installing..."
  case "$PM" in
    apt) sudo apt-get install -y rustscan 2>/dev/null || cargo install rustscan ;;
...
if command -v wpscan &>/dev/null; then
  ...
  warn "wpscan not found — installing..."
  case "$PM" in
    apt) sudo apt-get install -y wpscan 2>/dev/null || gem install wpscan ;;
...
if command -v gobuster &>/dev/null; then
  ...
  case "$PM" in
    apt) sudo apt-get install -y gobuster 2>/dev/null || go install github.com/OJ/gobuster/v3@latest ;;
...
if command -v feroxbuster &>/dev/null; then
  ...
  warn "feroxbuster not found — installing..."
  case "$PM" in
    apt) sudo apt-get install -y feroxbuster 2>/dev/null || cargo install feroxbuster ;;
...
if command -v arjun &>/dev/null; then
  ...
  pip3 install arjun --break-system-packages 2>/dev/null || pip3 install arjun && ok "arjun installed"
fi

# enum4linux
chk_install enum4linux enum4linux

# snmpwalk
if command -v snmpwalk &>/dev/null; then ok "snmpwalk already installed"
else install_pkg snmp && ok "snmpwalk installed"; fi

# nbtscan
chk_install nbtscan nbtscan

# medusa
chk_install medusa medusa

# crackmapexec
if command -v crackmapexec &>/dev/null; then
  ok "crackmapexec already installed"
else
  pip3 install crackmapexec --break-system-packages 2>/dev/null || warn "crackmapexec install failed — try: pip3 install crackmapexec"
fi

# sslscan
chk_install sslscan sslscan
...
if command -v theHarvester &>/dev/null; then
  ...
  case "$PM" in
    apt) sudo apt-get install -y theharvester 2>/dev/null ;;
    *) pip3 install theHarvester --break-system-packages 2>/dev/null ;;
  esac
fi

# shodan CLI
if command -v shodan &>/dev/null; then ok "shodan already installed"
else pip3 install shodan --break-system-packages && ok "shodan installed"; fi

# dnsrecon
if command -v dnsrecon &>/dev/null; then ok "dnsrecon already installed"
else
  case "$PM" in
    apt) sudo apt-get install -y dnsrecon 2>/dev/null ;;
    *) pip3 install dnsrecon --break-system-packages 2>/dev/null ;;
  esac
fi
