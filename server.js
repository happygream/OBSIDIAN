...
function buildArgs(module, target, domain, profile, auth, proxyArgs) {
  ...
  const map = {
    masscan:  { bin: 'bash',    args: ['-c', 'MSIP=$(dig +short ' + host + ' 2>/dev/null | grep -E "^[0-9]+\." | head -1); [ -z "$MSIP" ] && MSIP=' + host + '; masscan -p1-65535 $MSIP --rate=5000 --wait=3'] },
    nmap:     { bin: 'nmap',    args: [...(nmapFlagsMap[pf]||nmapFlagsMap.standard), '-Pn', host] },
    nikto:    { bin: 'nikto',   args: [
      '-h', target,   // use https target
      '-ssl',
...
function startServer(port) {
  return new Promise((resolve) => {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.send(JSON.stringify({ type: 'connected', version: '0.4.3' }));
