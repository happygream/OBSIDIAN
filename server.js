...
function parseFinding(module, line) {
  ...
  // sslscan
  if (module === 'sslscan') {
    if (/TLSv1\.0|SSLv|RC4|DES|NULL|EXPORT/i.test(line))
      return { sev: 'HIGH', title: 'Weak SSL/TLS', detail: line.trim() };
    if (/Heartbleed|POODLE|BEAST|ROBOT/i.test(line))
      return { sev: 'CRITICAL', title: 'SSL Vulnerability', detail: line.trim() };
    if (/Self-Signed|expired|not valid/i.test(line))
      return { sev: 'MEDIUM', title: 'Certificate Issue', detail: line.trim() };
  }
...
const TOOL_META = {
  ...
  // New tools v0.3.0
  rustscan:    { binary:'rustscan',    name:'rustscan',     desc:'Ultra-fast port scanner — feeds results to nmap',       install:'cargo install rustscan' },
  wpscan:      { binary:'wpscan',      name:'wpscan',       desc:'WordPress vulnerability scanner — themes, plugins, CVEs', install:'apt:wpscan / gem install wpscan' },
  gobuster:    { binary:'gobuster',    name:'gobuster',     desc:'Directory/DNS/vhost brute-force scanner',               install:'apt:gobuster / go install github.com/OJ/gobuster/v3@latest' },
  feroxbuster: { binary:'feroxbuster', name:'feroxbuster',  desc:'Recursive web content discovery',                      install:'apt:feroxbuster / cargo install feroxbuster' },
  arjun:       { binary:'arjun',       name:'arjun',        desc:'HTTP parameter discovery tool',                         install:'pip3 install arjun' },
  enum4linux:  { binary:'enum4linux',  name:'enum4linux',   desc:'SMB/NetBIOS enumeration for Windows targets',          install:'apt:enum4linux' },
  ...
  crackmapexec:{ binary:'crackmapexec',name:'crackmapexec', desc:'SMB/AD testing — pass-the-hash, shares enumeration',   install:'pip3 install crackmapexec' },
  sslscan:     { binary:'sslscan',     name:'sslscan',      desc:'Fast SSL/TLS scanner with detailed cipher analysis',   install:'apt:sslscan' },
  theharvester:{ binary:'theHarvester',name:'theHarvester', desc:'OSINT — emails, subdomains, IPs from public sources',  install:'apt:theharvester / pip3 install theHarvester' },
  shodan:      { binary:'shodan',      name:'shodan CLI',   desc:'Query Shodan for existing scan data on target',        install:'pip3 install shodan' },
  dnsrecon:    { binary:'dnsrecon',    name:'dnsrecon',     desc:'DNS enumeration — zone transfers, subdomain brute',    install:'apt:dnsrecon / pip3 install dnsrecon' },
};
...
async function runScan(rawConfig, ws) {
  ...
  // 3. REACHABILITY CHECK — non-blocking, runs in background while scan starts
...
function startServer(port) {
  return new Promise((resolve) => {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.send(JSON.stringify({ type: 'connected', version: '0.4.0' }));
