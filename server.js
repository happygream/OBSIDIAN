...
function buildArgs(module, target, domain, profile, auth, proxyArgs) {
  ...
  const map = {
    masscan:  { bin: 'bash',    args: ['-c', 'MSIP=$(dig +short -t A ' + host + ' 2>/dev/null | grep -E "^[0-9]+\." | grep -v "^;;" | head -1); if [ -z "$MSIP" ]; then echo "masscan: could not resolve ' + host + '"; exit 1; fi; masscan -p1-65535 $MSIP --rate=5000 --wait=3'] },
    ...
    nuclei:   { bin: 'nuclei',  args: [
      ...
      '-no-color',
      '-stats',                                  // show progress
      // -headless removed — requires libnss3 which may not be present
    ] },
    ...
    ssl:      { bin: 'testssl', args: ['--quiet', '--color', '0', domain] },
    ffuf:     { bin: 'ffuf',    args: [
      '-u', `${webTarget}/FUZZ`,
      '-w', pf === 'hardcore'
    ...
    wapiti:   { bin: 'bash',    args: ['-c', 'wapiti3 -u ' + target + ' -m xss,csrf,sql --level 2 -f txt -o /tmp/obsidian_wapiti 2>&1 || python3 -m wapitiCore.main -u ' + target + ' -m xss,csrf,sql --level 2 2>&1'] },
    ...
    sqlmap:   { bin: 'sqlmap',  args: [
      ...
      '--crawl=1',
      '--time-sec=5',
      ...(pf === 'hardcore' ? ['--level=3', '--risk=2', '--dbs', '--time-limit=300'] :
          pf === 'stealth'  ? ['--level=1', '--risk=1', '--time-limit=120'] :
                              ['--level=1', '--risk=1', '--time-limit=120']),
      '--batch',
      '--output-dir', '/tmp/obsidian_sqlmap',
      '--flush-session',
    ] },
  };
  ...
  if (module === 'hydra' && auth) {
    ...
    const ul  = sanitiseFilePath(auth.userlist) || '/usr/share/dirb/wordlists/others/names.txt';
    const pl  = sanitiseFilePath(auth.passlist)  || '/usr/share/dirb/wordlists/others/best1050.txt';
    ...
    let hydraArgs = [];
    if (mode === 'stuffing') {
      hydraArgs = ['-L', '/usr/share/dirb/wordlists/others/names.txt', '-P', '/usr/share/dirb/wordlists/others/best1050.txt'];
    } else if (mode === 'brute') {
      hydraArgs = ['-L', '/usr/share/dirb/wordlists/others/names.txt', '-P', '/usr/share/dirb/wordlists/others/best1050.txt'];
    } else {
      ...
      hydraArgs = ['-L', '/usr/share/dirb/wordlists/others/names.txt', '-p', commonPasswords[0]];
    }
  ...
  map.rustscan = { bin: 'rustscan', args: [
    ...
    '--greppable',  // machine-readable output, no ANSI art
    '--', '-sV', '-sC',
  ] };
...
function startServer(port) {
  return new Promise((resolve) => {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.send(JSON.stringify({ type: 'connected', version: '0.4.4' }));
