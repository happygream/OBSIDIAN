...
// ===== MODULES =====
const MODS = [
  {id:'masscan',name:'masscan',tag:'ports'},{id:'nmap',name:'nmap',tag:'svc'},
  {id:'nikto',name:'nikto',tag:'web'},{id:'nuclei',name:'nuclei',tag:'cve'},
  {id:'headers',name:'headers',tag:'http'},{id:'ssl',name:'testssl',tag:'tls'},
  {id:'ffuf',name:'ffuf',tag:'fuzz'},{id:'amass',name:'amass',tag:'recon'},
  {id:'subfinder',name:'subfinder',tag:'recon'},{id:'wapiti',name:'wapiti',tag:'inject'},
  {id:'hydra',name:'hydra',tag:'auth'},{id:'whatweb',name:'whatweb',tag:'fp'},
  {id:'sqlmap',name:'sqlmap',tag:'sqli'},
];
const DEFAULT_ON = new Set(['nmap','headers','ffuf','whatweb']);
...
function addFinding(f) {
  ...
  const card = document.createElement('div'); card.className = 'f-card ' + f.sev; card.onclick = () => openFinding(fidx);
...
function exportHTML() {
  const sc = {CRITICAL:'#ff2255',HIGH:'#ff6600',MEDIUM:'#ffe600',LOW:'#0088ff',INFO:'#7a5a7a'};
  const rows = findings.map(f => '<tr><td style="color:'+sc[f.sev]+';font-weight:bold">'+f.sev+'</td><td>'+(f.module||'')+'</td><td>'+f.title+'</td><td>'+f.detail+'</td></tr>').join('');
  const css = 'body{background:#060408;color:#e8d0e8;font-family:monospace;padding:32px;max-width:1000px;margin:0 auto}h1{color:#ff00c8;letter-spacing:5px}.m{font-size:11px;color:#7a5a7a;margin:3px 0}.sm{display:flex;flex-wrap:wrap;gap:12px;margin:16px 0}.sc{border:1px solid rgba(255,0,200,0.2);padding:12px 18px;text-align:center}.sc .n{font-size:26px;font-weight:700}.sc .l{font-size:9px;letter-spacing:2px;color:#7a5a7a}table{width:100%;border-collapse:collapse;font-size:11px;margin-top:18px}th{background:rgba(255,0,200,0.1);color:#ff00c8;padding:9px 12px;text-align:left;font-size:9px}td{padding:9px 12px;border-bottom:1px solid rgba(255,0,200,0.07)}footer{margin-top:32px;font-size:10px;color:#3a2a3a;text-align:center;border-top:1px solid rgba(255,0,200,0.08);padding-top:12px}';
  const sevs = ['CRITICAL','HIGH','MEDIUM','LOW','INFO'].map(s => '<div class="sc"><div class="n" style="color:'+sc[s]+'">'+findings.filter(f=>f.sev===s).length+'</div><div class="l">'+s+'</div></div>').join('');
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>OBSIDIAN Report</title><style>'+css+'</style></head><body><h1>OBSIDIAN</h1><div class="m">Target: '+scanTarget+'</div><div class="m">Generated: '+gts()+'</div><div class="sm">'+sevs+'</div><table><thead><tr><th>SEV</th><th>MODULE</th><th>TITLE</th><th>DETAIL</th></tr></thead><tbody>'+rows+'</tbody></table><footer>OBSIDIAN &mdash; '+gts()+'</footer></body></html>';
  dl(new Blob([html],{type:'text/html'}),'obsidian-report.html');
}

function exportPDF() {
  const sc = {CRITICAL:'#cc0033',HIGH:'#cc5500',MEDIUM:'#997700',LOW:'#005599',INFO:'#666'};
  const rows = findings.map(f => '<tr><td style="color:'+sc[f.sev]+';font-weight:bold">'+f.sev+'</td><td>'+(f.module||'')+'</td><td>'+f.title+'</td><td>'+f.detail+'</td></tr>').join('');
  const css = 'body{font-family:sans-serif;color:#111;padding:28px;max-width:900px;margin:0 auto;font-size:12px}h1{font-size:20px;letter-spacing:3px;color:#1a001a;border-bottom:2px solid #ff00c8;padding-bottom:6px}.m{font-size:11px;color:#888;margin:3px 0}.sm{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0}.sc{border:2px solid #1a001a;padding:10px 16px;text-align:center;border-radius:3px}.sc .n{font-size:22px;font-weight:bold}.sc .l{font-size:9px;letter-spacing:2px;color:#888}table{width:100%;border-collapse:collapse;font-size:11px;margin-top:14px}th{background:#1a001a;color:#fff;padding:7px 10px;text-align:left;font-size:9px}td{padding:7px 10px;border-bottom:1px solid #eee}tr:nth-child(even)td{background:#fdf8ff}footer{margin-top:28px;font-size:10px;color:#aaa;text-align:center;border-top:1px solid #ddd;padding-top:10px}';
  const sevs = ['CRITICAL','HIGH','MEDIUM','LOW','INFO'].map(s => '<div class="sc"><div class="n" style="color:'+sc[s]+'">'+findings.filter(f=>f.sev===s).length+'</div><div class="l">'+s+'</div></div>').join('');
  const w = window.open('','_blank');
  w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}'+css+'</style></head><body><h1>OBSIDIAN</h1><div class="m">Target: '+scanTarget+'</div><div class="m">Generated: '+gts()+'</div><div class="sm">'+sevs+'</div><table><thead><tr><th>SEV</th><th>MODULE</th><th>TITLE</th><th>DETAIL</th></tr></thead><tbody>'+rows+'</tbody></table><footer>OBSIDIAN &mdash; '+gts()+'</footer><script>window.onload=function(){window.print();}<\/script></body></html>');
  w.document.close();
}
...
function renderHistory() {
  ...
  list.innerHTML = history.map(e => {
    ...
    return '<div style="background:rgba(255,0,200,0.03);border:1px solid rgba(255,0,200,0.15);border-radius:3px;padding:12px 14px;cursor:pointer;" onclick="loadHistoryEntry('+e.id+')">'
