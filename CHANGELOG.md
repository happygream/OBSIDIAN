...
### Fixed
- sqlmap: removed `--max-targets` (not a valid flag in sqlmap 1.10.4)
- gobuster: replaced `--wildcard` (invalid flag) with `--exclude-length 0`
- masscan: uses `dig -t A` for reliable IPv4 resolution, exits cleanly if unresolvable
- nikto: now scans `https://domain` correctly
- arjun: output path was Windows temp dir — removed output file arg
- dnsrecon: removed `--xml` Windows path arg, removed `brt` type (broken on Python 3.12 / Ubuntu 26.04)
- hydra/medusa: wordlist paths updated to `/usr/share/wordlists/john.lst` (rockyou.txt not present)
- nbtscan: resolves hostname to IP before scanning — was failing on domain input
- arjun: now tries `python3 -m arjun` as fallback if binary not in PATH

## [0.4.3] — 2026-06-08

### Fixed
- masscan: now resolves hostname to IP via `dig` before scanning — fixes "unknown parameter" error
- nikto: now scans `https://` target instead of `http://` — was finding "no web server on port 80"
- nuclei: removed `--headless` flag — was downloading Chromium then failing on missing `libnss3`
- sqlmap: `--output-dir` now uses `/tmp/obsidian_sqlmap` — was leaking Windows path into WSL
- gobuster: added `--wildcard` flag — fixes early exit on Cloudflare wildcard responses
- rustscan: added `--greppable` flag — eliminates ANSI colour art from terminal output
- whatweb: install method changed from `apt:whatweb` to `gem-dev:whatweb` — apt package is broken
