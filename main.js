...
app.whenReady().then(() => {
  ...
  // Create window immediately — don't wait for server
  createWindow();
  setupUpdater();

  // Start server in background — renderer connects via WebSocket when ready
  startServer(serverPort).catch(err => {
    console.error('[OBSIDIAN] Server failed to start:', err.message);
  });
