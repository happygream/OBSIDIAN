...
- Tool detection rewritten — reliable bash check across all known binary locations
- Profile selection instantly updates module checkboxes with notification
- rustscan/feroxbuster install via prebuilt GitHub release binaries
- All apt installs run as `wsl -u root` — no sudo prompts
- masscan, nikto, gobuster, sqlmap, arjun, dnsrecon, nbtscan scan args fixed from live testing
- wordlists auto-install via dirb on first launch
- `python3 -m` fallback for wapiti, theHarvester, shodan, arjun
